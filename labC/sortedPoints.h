...
/*
  * Add a node, new, before a node, ref.  Returns 1
  * if ref points at something not in sp, 0 otherwise.
  */  
int insertBefore(SortedPoints *sp, Node *new, Node *ref);

/*
  * Add a node, new, after a node, ref.
  */  
void insertAfter(Node *new, Node *ref);

/*
  * Delete the node pointed to by del
  */ 
void deleteNode(SortedPoints *sp, Node *del);

/*
 * Print out a a SortedPoints list.
 */
